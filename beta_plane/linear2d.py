...
class LinearShallowWater(object):
    def __init__(self, nx, ny, Lx=1.0e7, Ly=1.0e7, f0=0, beta=2.0e-11, H=100.0, g=0.05, nu=1.0e-5, dt=1000.0, bcond='periodicx'):
    ...
    def diff2x(self, phi):
        ...
        return (phi[:-2, :] - phi[1:-1, :] + phi[2:, :]) / self.dx**2
    ...
    def diff2y(self, phi):
        ...
        return (phi[:, :-2] - phi[:, 1:-1] + phi[:, 2:]) / self.dy**2
    ...
    def rhs(self):
        ...
        # the height equation
        h_rhs = -H*self.divergence() + nu*self.del2(self._h) # - self.damping(self._h)[1:-1, 1:-1]

        # the u equation
        dhdx = self.diffx(self._h)[:, 1:-1]
        u_rhs = (f0 + beta*self.uy)*vv - g*dhdx + nu*self.del2(self._u) # - self.damping(self._u)[1:-1, 1:-1]
        
        # the v equation
        dhdy  = self.diffy(self._h)[1:-1, :]
        v_rhs = -(f0 + beta*self.vy)*uu - g*dhdy + nu*self.del2(self._v) # - self.damping(self._v)[1:-1, 1:-1]
...
ocean = LinearShallowWater(nx, ny, Lx, Ly, beta=beta, f0=0.0, g=0.1, H=100.0, dt=3000, bcond='wallsx')
